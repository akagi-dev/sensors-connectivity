...
function testConfig(overrides: Partial<PubsubBroadcasterConfig> = {}): PubsubBroadcasterConfig {
  return {
...
describe('pubsub broadcaster integration flow (mock harness)', () => {
  it('processes consume -> publish -> result event -> commit in order', async () => {
    ...
    const fakeConsumer = {
      ...
      async run({ eachBatch }: { eachBatch: (payload: FakeBatchPayload) => Promise<void> }) {
        await eachBatch({
          batch: {
  ...
  it('routes exhausted transient failures to DLQ and commits afterward', async () => {
    ...
    const service = createPubsubBroadcasterService(testConfig({ maxRetries: 2 }), {
      createConsumer: () => fakeConsumer as unknown as Consumer,
      createPublisher: () => ({
        async connect() {},
        async disconnect() {},
