...
export function loadEndpointConfig(env: NodeJS.ProcessEnv = process.env): EndpointConfig {
  return {
    ...
    timestampSkewSeconds: parsePositiveInt(env.ENDPOINT_TIMESTAMP_SKEW_SECONDS, 300),
    producerMaxAttempts: parsePositiveInt(env.ENDPOINT_PRODUCER_MAX_ATTEMPTS, 3),
    producerRetryBackoffMs: parsePositiveInt(env.ENDPOINT_PRODUCER_RETRY_BACKOFF_MS, 100)
  };
}
