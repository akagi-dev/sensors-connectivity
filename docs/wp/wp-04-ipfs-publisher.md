...
## Scope / Goal
Deliver `ipfs-publisher` as an idempotent Kafka consumer that deterministically batches authorized telemetry, publishes/pins to IPFS, and emits `telemetry.ipfs.published.v1` events containing the CID and batch event count.
...
- Kafka topic produced: `telemetry.ipfs.published.v1` payload with:
  - `cid`
  - `event_count`
...
- [ ] Emit `telemetry.ipfs.published.v1` with canonical envelope and payload (`cid`, `event_count`).
...
- Only commit offset after successful pin + `telemetry.ipfs.published.v1` emission.
...
- Contract tests:
  - consumed `telemetry.authorized.v1` schema and produced `telemetry.ipfs.published.v1` payload compatibility.
...
- Published `telemetry.ipfs.published.v1` events consistently contain valid `cid` and `event_count` after successful pin.
